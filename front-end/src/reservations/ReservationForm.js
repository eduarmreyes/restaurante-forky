...
function ReservationForm() {
  ...
  function phoneValidation() {
    ...
    if (regFull.test(mobileNumber)) {
      ...
      setReservationsError({
        message: "The phone number does not meet format criteria",
      });
      return false;
    }
  }
  ...
  async function onSubmit(e) {
    ...
    try {
      if (dateValidation() && phoneValidation()) {
        ...
        if (params.reservation_id) {
          ...
          response = await sendUpdate(
            { data: reserveObj },
            "reservations/" + reserveObj.reservation_id,
            abortController.signal
          );
      ...
      setReservationsError({
        message:
          "Make sure the date is greater than today and all inputs are correct",
      });
      console.log(e);
    }
  }
